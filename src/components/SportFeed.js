...
const SportFeed = ({ sport }) => {
  ...
  useEffect(() => {
    if (sport) {
      fetch(`http://fanfeed-server-env.eba-wnhhgsmd.us-west-2.elasticbeanstalk.com/api/news/${sport}?sortBy=${sortBy}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (sport) {
      fetch(`http://fanfeed-server-env.eba-wnhhgsmd.us-west-2.elasticbeanstalk.com/api/bluesky/${sport}?sort=${sortByBS}`)
        .then((res) => res.json())
        .then((data) => {
